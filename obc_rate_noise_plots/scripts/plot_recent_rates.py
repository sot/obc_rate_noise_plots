...
def get_options():
    ...
    parser = OptionParser()
    parser.set_defaults()
    parser.add_option("--start", help="Start time")
    parser.add_option("--stop", help="Stop time")
    parser.add_option(
        "--duration", type="float", default=1.6e7, help="Plot duration (seconds)"
    )
    parser.add_option("--outdir", default=".")
    parser.add_option("--root", default="recent", help="Root file name")
...
def main():
    ...
    tstop = DateTime(
        opt.stop
    ).secs  # if opt.stop is None then this returns current time
    ...
    if "msids" not in globals():
        ...
        msids = fetch.MSIDset(
            ["aorate1", "aorate2", "aorate3"], tstart, tstop, filter_bad=True
        )
    ...
    if 1:
        ...
        for label, rates in (
            ...
            plt.clf()
            plot_cxctime(
                times, rates, ".", markersize=2, mew=0, color="blue", alpha=0.20
            )
            plt.ylim(-4, 4)
